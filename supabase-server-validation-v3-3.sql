-- =========================================================
-- Fix v3.3: unifica tempo entre popup, ranking e PGN
-- Roda no Supabase Dashboard → SQL Editor
-- =========================================================
-- Problema: 3 medidas de tempo diferentes apareciam em 3
-- lugares (popup 4:51, ranking 4:52, PGN 4:59) e confundiam
-- o usuario.
--
-- Fix: register_ai_score agora aceita p_client_player_seconds
-- e usa como display em qualquer caso (nao so' client_validated),
-- desde que o valor seja plausivel (entre 1 e total_game_ms/1000
-- com 5s de tolerancia). v_total_ms continua server-validated
-- pra anti-cheat (variance, etc).
...
begin
  ...
  -- =============== UNIFICACAO DO TEMPO ===============
  -- Cliente passa o tempo dele (lastWinData.player_seconds) — tempo
  -- de pensamento puro do jogador, excluindo o que a IA pensou.
  -- Usa esse valor pro display (ranking, PGN) desde que seja plausivel.
  if p_client_validated then
    -- Pos-login: bulk-INSERT zerou v_total_ms, usa client direto
    ...
    -- Server-validated: usa client player_seconds se for plausivel
    -- (entre 1 e total_game_ms/1000 com 5s de tolerancia)
    if p_client_player_seconds is not null
       and p_client_player_seconds > 0
       and p_client_player_seconds <= (v_total_ms / 1000 + 5) then
      v_player_seconds := p_client_player_seconds;
      -- v_total_ms continua igual (wall-clock real do server) pra anti-cheat
    end if;
  end if;
