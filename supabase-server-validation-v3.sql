-- =========================================================
-- Anti-cheat v3: post-game registration (logged-out friendly)
-- Roda no Supabase Dashboard → SQL Editor
-- =========================================================
-- Problema resolvido: usuario que joga deslogado nao conseguia
-- registrar score (RPC dava 'no moves found'). Agora salvamos
-- os lances localmente e fazemos bulk-insert apos login,
-- marcando o score com client_validated=true (validacao parcial).
--
-- Trade-off: scores client_validated tem timing supplied pelo
-- cliente (nao server-trusted). Recomendado nao pagar premio
-- em dinheiro para scores client_validated. Para ranking
-- cosmetico, esta tudo bem.

-- Nova coluna em scores
alter table scores add column if not exists client_validated boolean default false;
create index if not exists idx_scores_client_validated on scores(client_validated);

-- Drop a versao antiga da funcao (assinatura mudou)
drop function if exists register_ai_score(text, smallint, text, int, int, int, int, int, int);

-- Recria com client_validated
...
begin
  ...
  -- HEURISTICAS BASE (server timing — quando server-validated)
  if not p_client_validated then
    -- Server-side timing so vale quando lances foram inseridos durante o jogo
    ...
    -- Client-validated: nao podemos confiar em timing. Pequena suspeita
    -- automatica + skip das checagens de timing server-side.
    v_suspicion := v_suspicion + 1;
  end if;

  -- Client-side signals (sempre aplicados)
  ...
  -- Comparacao intra-usuario (so Expert)
...
grant execute on function register_ai_score(text, smallint, text, int, int, int, int, int, int, boolean) to authenticated;
